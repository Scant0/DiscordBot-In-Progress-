...
class Purge(commands.Cog):
    ...
    async def _ack(self, ctx: commands.Context, message: str):
        ...
        # Prefix command: send then auto-delete (not reply to avoid referencing a deleted message)
        try:
            msg = await ctx.send(message, allowed_mentions=discord.AllowedMentions.none())
